...
### Phase 6 — 演示收口与作品集包装（面试向）

**目标：** 可复现、话术与仓库一致；录屏可选（非盈利产品化）。

| 项 | 内容 | 状态 |
|----|------|------|
| 金路径 README | MQ → AUTO → STABLE → 控制台问答；可选 RAG/MCP | ✅ `middleware-demo/demo/README.md` |
| 简历条目草稿 | 已完成 vs 可选加分写清 | ✅ 同上 §6 |
| 录屏 | 5～8 分钟走金路径 | 可选（面试附件） |
| 从零 Compose 全家桶 | 非面试必需 | 刻意不做大而全 |

**Phase 6 DoD**

- [x] 按金路径 README 可从零演示主闭环（`middleware-demo/demo/README.md`）
- [x] 简历项目描述草稿与仓库一致（已完成 vs 可选加分写清）
- [ ] （可选）自行录屏附件
