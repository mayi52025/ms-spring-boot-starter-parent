...
**自动配置说明**：
- 系统会自动获取应用名称（spring.application.name）作为服务名称
- 系统会自动获取本机 IP 地址作为服务 IP
- 系统会自动获取服务器端口（server.port）作为服务端口
- 系统会在应用启动时自动注册到 Nacos
- 无需手动调用注册方法，服务会自动注册
...
**自动配置说明**：
- 系统会自动获取应用名称（spring.application.name）构建配置 ID
- 配置 ID 格式：${spring.application.name}.${file-extension}
- 系统会在应用启动时自动拉取配置
- 系统会自动监听配置变更，实时响应配置更新
- 无需手动调用获取配置方法，配置会自动加载
