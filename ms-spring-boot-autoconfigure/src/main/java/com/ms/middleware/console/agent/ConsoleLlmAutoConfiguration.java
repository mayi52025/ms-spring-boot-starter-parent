package com.ms.middleware.console.agent;

import com.ms.middleware.autonomy.insight.tool.MiddlewareInsightTool;
import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

/**
 * Phase 5.1：LangChain4j Agent + Insight Tool。
 */
@Configuration(proxyBeanMethods = false)
@ConditionalOnProperty(prefix = "ms.middleware.console", name = "llm-enabled", havingValue = "true")
public class ConsoleLlmAutoConfiguration {

    @Bean
    public MiddlewareInsightLangChainTools middlewareInsightLangChainTools(MiddlewareInsightTool insightTool) {
        return new MiddlewareInsightLangChainTools(insightTool);
    }
}
